...
exports.router.get('/document/:id', async (req, res, next) => {
    try {
        const tx = new mssql_1.MSSQL(sql_pool_jetti_1.JETTI_POOL);
        let noSqlDoc = null;
        const flatDoc = await std_lib_1.lib.doc.byId(req.params.id, tx);
        if (flatDoc) {
            noSqlDoc = await std_lib_1.lib.doc.noSqlDocument(flatDoc);
            delete noSqlDoc.doc['serverModule'];
            noSqlDoc.docByKeys = Object.keys(noSqlDoc.doc)
                .map(key => ({ key: key, value: noSqlDoc.doc[key] }));
            if (req.query.asArray === 'true')
                res.json(Object.entries(flatDoc));
        }
        res.json(noSqlDoc);
    }
    catch (err) {
...
exports.router.get('/document/meta/:type/:operationId', async (req, res, next) => {
    try {
        const tx = new mssql_1.MSSQL(sql_pool_jetti_1.JETTI_POOL);
        const flatDocument = { id: (await std_lib_1.lib.util.GUID()), Operation: req.params.operationId || null };
        const docServer = await std_lib_1.lib.doc.createDocServer(req.params.type, flatDocument, tx);
...
//   } catch (err) { next(err); }
// });
exports.router.post('/document', async (req, res, next) => {
    try {
        const sdb = db_sessions_1.SDB(req);
        await sdb.tx(async (tx) => {
            await std_lib_1.lib.util.adminMode(true, tx);
            try {
                const { document, options } = JSON.parse(JSON.stringify(req.body), jetti_middle_1.dateReviverUTC);
                res.statusCode = 405;
                if (!document.type) {
                    res.json(`Document type not specified`);
                    return;
                }
                if (!options.searchKey && options.updateType !== 'Insert') {
                    res.json(`Search key is not specified`);
                    return;
                }
                let docServer = null;
                let flatDocument = null;
                const existDoc = await std_lib_1.lib.doc.findDocumentByKey(options.searchKey, tx);
                ...
                else if (!existDoc && options.updateType.includes('Insert'))
                    res.statusCode = 200;
                else
                ...
                if (res.statusCode !== 200)
                    return;
                if (!flatDocument) {
                    flatDocument = { id: document.id || (await std_lib_1.lib.util.GUID()) };
                    if (document.type === 'Document.Operation') {
                        flatDocument.Operation = document.doc.Operation;
                        if (!flatDocument.Operation) {
                            res.json(`Value of required field 'Operation' are not filled`);
                            return;
                        }
                    }
                }
                docServer = await std_lib_1.lib.doc.createDocServer(document.type, flatDocument, tx);
                res.statusCode = 405;
                const props = docServer.Props();
                ...
                if (emptyProps.length) {
                    res.json(`Values of required fields are not filled: ${emptyProps}`);
                    return;
                }
                if (docServer.type === 'Document.Operation' && !docServer['Operation']) {
                    res.json(`Value of required field 'Operation' are not filled`);
                    return;
                }
                if (options.commands) {
                ...
                await std_lib_1.lib.doc.saveDoc(docServer, tx, options.queueFlow, { withExchangeInfo: !!(docServer['ExchangeBase'] || docServer['ExchangeCode']) });
                const docByKeys = Object.keys(docServer['doc']).map(key => ({ key: key, value: docServer['doc'][key] }));
                delete docServer['doc'];
                res.statusCode = 200;
                const noSqlDoc = await std_lib_1.lib.doc.noSqlDocument(docServer);
...
exports.router.delete('/document/:id', async (req, res, next) => {
    try {
        const sdb = db_sessions_1.SDB(req);
        await sdb.tx(async (tx) => {
            await std_lib_1.lib.util.adminMode(true, tx);
        ...
        `, [id, serverDoc.date, serverDoc.deleted, 0]);
                ...
                res.status(500).json(Object.assign(Object.assign({}, ex), { Error: ex.message }));
            }
            finally {
                await std_lib_1.lib.util.adminMode(false, tx);
            }
...
exports.router.post('/executeOperation', async (req, res, next) => {
    try {
        const sdb = db_sessions_1.SDB(req);
        await sdb.tx(async (tx) => {
            await std_lib_1.lib.util.adminMode(true, tx);
            try {
                ...
                let oper = null;
                if (!operationId)
                    err = 'Empty arg "operationId"';
                else
                    oper = await std_lib_1.lib.doc.createDocServerById(operationId, tx);
                if (!oper)
                    err = `Operation with id ${operationId} does not exist`;
                else {
                    let result = {};
                    const docModule = oper['serverModule'][args.method || 'RESTMethod'];
                    if (typeof docModule === 'function')
                        result = await docModule(args);
                    res.json(result);
                }
            }
            catch (ex) {
