...
async function List(params, tx) {
    ...
    if (params.listOptions && params.listOptions.withHierarchy) {
        let parent = null;
        if (params.id) {
            const ob = await std_lib_1.lib.doc.byId(params.id, tx);
            parent = ob.isfolder && params.listOptions.hierarchyDirectionUp ? params.id : ob.parent;
        }
        // folders
        const queryList = config_1.configSchema().get(params.type).QueryList;
        ...
        if (parent) {
            const ancestors = await std_lib_1.lib.doc.Ancestors(params.id, tx);
        ...
        params.filter = [];
        params.filter.push(new jetti_middle_1.FormListFilter('parent', '=', { id: parent, type: params.type }));
        params.filter.push(new jetti_middle_1.FormListFilter('isfolder', '=', 0));
        if (deletedFilter)
            params.filter.push(new jetti_middle_1.FormListFilter('deleted', '=', 0));
    ...
    const cs = config_1.configSchema().get(params.type);
    const { QueryList, Props } = cs;
    let row = null;
    let query = '';
    if (params.id)
        row = (await tx.oneOrNone(`SELECT * FROM (${QueryList}) d WHERE d.id = '${params.id}'`));
    if (!row && params.command !== 'last')
    ...
    const queryBuilder = (isAfter) => {
      ...
      WHERE ${(filterBuilder_1.filterBuilder(params.filter).where)} AND (`;
        valueOrder.forEach(_or => {
            let where = '(';
            order.forEach(_o => where += ` "${_o.field}" ${_o !== order[order.length - 1] ? '=' :
    ...
    const queryFilter = filterBuilder_1.filterBuilder(params.filter);
...
exports.List = List;
function listPostProcess(data, params) {
    let result = [];
