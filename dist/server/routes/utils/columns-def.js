...
function buildColumnDef(view, settings) {
    ...
    Object.keys(view).filter(property => view[property] && view[property]['type'] !== 'table').map((field) => {
        ...
        columnDef.push({
            ...
            filter: settings.filter.find(f => f.left === field) || new user_settings_1.FormListFilter(field),
            sort: settings.order.find(f => f.field === field) || new user_settings_1.FormListOrder(field),
