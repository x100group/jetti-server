...
exports.SQLGenegatorMetadata = void 0;
...
class SQLGenegatorMetadata {
    static typeSpliter(type, begin, length = 30) {
        return `\n${'-'.repeat(length)} ${begin ? 'BEGIN' : 'END'} ${type} ${'-'.repeat(length)}\n`;
    }
    static QueryRegisterAccumulationView(doc, type) {
        let select = '';
        let fields = '';
        ...
        for (const prop in jetti_middle_1.excludeRegisterAccumulatioProps(doc)) {
            fields += `, [${prop}]`;
            const propType = doc[prop].type || 'string';
    ...
    static CreateRegisterAccumulationViewIndex() {
        let query = '';
        for (const type of factory_1.RegisteredRegisterAccumulation) {
            const register = factory_1.createRegisterAccumulation({ type: type.type });
    ...
    static QueryRegisterIntoView(doc, type) {
        const simleProperty = (prop, type) => {
            switch (type) {
                case 'boolean':
                    return `
        ...
        const complexProperty = (prop) => `
        , TRY_CONVERT(UNIQUEIDENTIFIER, JSON_VALUE(data, N'$."${prop}"')) [${prop}]`;
        let insert = '';
        let select = '';
        let fields = '';
        const Props = jetti_middle_1.excludeRegisterAccumulatioProps(doc);
    ...
    static CreateRegisterInfoViewIndex() {
        let query = '';
        for (const type of factory_2.GetRegisterInfo()) {
            const register = factory_2.createRegisterInfo({ type: type.type });
    ...
    static async CreateViewOperations() {
        const operations = global_1.Global.indexedOperations();
        let query = '';
        for (const operation of operations) {
            const indexedOperation = indexedOperation_1.getIndexedOperationById(operation[0]);
            if (!indexedOperation)
                continue;
    ...
    static CreateViewOperation(operation, asArrayOfQueries = false) {
        const subQueries = [];
        const type = operation.type;
        let select = global_1.Global.configSchema().get(operation.type).QueryList;
        select = select
    ...
    static async CreateViewOperationsIndex(withSecurityPolicy = true) {
        const tx = std_lib_1.lib.util.jettiPoolTx();
        const operations = global_1.Global.indexedOperations();
        let query = '';
        for (const operation of operations) {
            const indexedOperation = indexedOperation_1.getIndexedOperationById(operation[0]);
    ...
    static async CreateViewOperationIndex(operation, tx, asArrayOfQueries = false, withSecurityPolicy = true) {
        const subQueries = [];
        const type = operation.type;
        const doc = await std_lib_1.lib.doc.createDocServer('Catalog.Operation', { id: operation.id, Operation: operation.id }, tx);
        const Props = (await doc.getPropsFunc(tx))();
        const select = jetti_middle_1.SQLGenegator.QueryListRaw(Props, type)
    ...
    GO`;
        const registeredCatalogs = [...documents_factory_1.RegisteredDocuments().values()].filter(e => !jetti_middle_1.Type.isOperation(e.type) && e.dynamic === dynamic);
    ...
    static CreateViewCatalog(type, asArrayOfQueries = false) {
        const subQueries = [];
        const doc = documents_factory_1.createDocument(type);
        if (!doc['QueryList']) {
            const Props = doc.Props();
            const type = doc.Prop().type;
            let select = jetti_middle_1.SQLGenegator.QueryList(Props, doc.type);
            const typeSplit = type.split('.');
            let name = '';
            for (let i = 1; i < typeSplit.length; i++)
                name += typeSplit[i];
    ...
    static CreateViewCatalogsIndex(withSecurityPolicy = true, dynamic = false) {
        ...
        let query = '';
        for (const registeredCatalog of registeredCatalogs) {
            const doc = documents_factory_1.createDocument(registeredCatalog.type);
            if (doc['QueryList'])
                continue;
...
RAISERROR('${registeredCatalog.type} end', 0 ,1) WITH NOWAIT;
    ...
    static CreateViewCatalogIndex(type, withSecurityPolicy = true, asArrayOfQueries = false) {
        const subQueries = [];
        const doc = documents_factory_1.createDocument(type);
        if (!doc['QueryList']) {
            const Props = doc.Props();
            const select = jetti_middle_1.SQLGenegator.QueryListRaw(Props, doc.type);
            if (withSecurityPolicy)
                subQueries.push(`
BEGIN TRY
  ALTER SECURITY POLICY[rls].[companyAccessPolicy] DROP FILTER PREDICATE ON[dbo].[${type}.v];
...
END CATCH;`);
            ...
            subQueries.push(`CREATE UNIQUE CLUSTERED INDEX [${type}.v] ON [${type}.v](id);${Object.keys(Props)
                ...
                .join('\n')}
        ${jetti_middle_1.Type.isDocument(doc.type) ? `
...
ALTER SECURITY POLICY [rls].[companyAccessPolicy] ADD FILTER PREDICATE [rls].[fn_companyAccessPredicate]([company]) ON [dbo].[${type}.v];`);
    ...
    static CreateDocumentIndexes() {
        const types = {
            catalogs: [...documents_factory_1.RegisteredDocuments().values()].filter(t => jetti_middle_1.Type.isCatalog(t.type)),
            documents: [...documents_factory_1.RegisteredDocuments().values()].filter(t => jetti_middle_1.Type.isDocument(t.type))
        };
        ...
        for (const catalog of types.catalogs) {
            const doc = documents_factory_1.createDocument(catalog.type);
            if (doc['QueryList'])
                continue;
            select += `
    ...
    static CreateTableRegisterAccumulationTO() {
        ...
        let query = '';
        for (const register of factory_1.RegisteredRegisterAccumulation) {
            const doc = factory_1.createRegisterAccumulation({ type: register.type });
            const props = doc.Props();
            let select = '';
            let groupBy = '';
            let indexGroupBy = '';
    ...
    static RegisterAccumulationClusteredTable(doc, type) {
        const simleProperty = (prop, type) => {
            if (type === 'boolean') {
                return `
        , [${prop}] BIT N'$.${prop}'`;
    ...
    static RegisterAccumulationViewQuery(doc, type) {
        const simleProperty = (prop, type) => {
            if (type === 'boolean') {
                return `
        , [${prop}] BIT N'$.${prop}'`;
...
exports.SQLGenegatorMetadata = SQLGenegatorMetadata;
//# sourceMappingURL=SQLGenerator.MSSQL.Metadata.js.map