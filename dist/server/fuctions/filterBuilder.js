...
exports.filterBuilder = (filter, excludesTypes = ['Catalog.Operation.Group', 'Catalog.User', 'Catalog.Operation']) => {
    let where = ' (1 = 1) ';
    let tempTable = '';
    const filterList = filter
        .filter(f => !(f.right === null || f.right === undefined))
        .map(f => (Object.assign(Object.assign({}, f), { leftQ: `\"${f.left}\"` })));
        ...
        switch (f.center) {
            ...
            case '<>':
                ...
                if (f.right instanceof Date) {
                    where += ` AND ${f.leftQ} ${f.center} '${f.right.toJSON()}'`;
                    break;
                }
                if (typeof f.right === 'object') {
                    if (!f.right.id)
                        where += ` AND ${f.leftQ} IS NULL `;
                    else if (f.left === 'parent.id')
                        where += ` AND ${f.leftQ} = '${f.right.id}'`;
                    else if (excludesTypes.includes(f.right.type))
                        where += ` AND ${f.leftQ} = '${f.right.id}'`;
