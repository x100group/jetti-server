...
exports.BankStatementUnloader = void 0;
const std_lib_1 = require("../std.lib");
class BankStatementUnloader {
    static isUKRAINE() {
    ...
    static getQueryTextSalaryProjectCommon() {
        if (this.isUKRAINE()) {
            return `
      select
        sp.code N'N_D',
    ...
    static getQueryTextSalaryEmployees() {
        return `
    ...
    static getQueryTextSalaryEmployeesUKRAINE() {
        return `
    DROP TABLE IF EXISTS #PayRolls;
    SELECT *
    INTO #PayRolls
    ...
    static async getQueryTextCommon() {
        ...
        const operation = await std_lib_1.lib.doc.byId(this.operation, this.tx);
        throw new Error(`Не реализована выгрузка для операции ${operation.description}`);
    }
    static getQueryTextCommonUKRAINEОплатаПоставщику() {
        return `SELECT
    Obj.[code] as N_D
    ...
    static getQueryTextCommonUKRAINEНаРасчетныйСчетВПуть() {
        return `SELECT
        Obj.[code] as N_D
        ,CONVERT(MONEY, JSON_VALUE(Obj.doc, '$.Amount')) as SUMMA
        ,BAComp.[code] as COUNT_A
    ...
    static getQueryTextCommonUKRANEВозвратКредитовИЗаймов() {
        return `SELECT
    Obj.[code] as N_D
    ,CONVERT(MONEY, JSON_VALUE(Obj.doc, '$.Amount')) as SUMMA
    ,BAComp.[code] as COUNT_A
  ...
  order by Obj.company, BAComp.[code], Obj.[date]`;
    ...
    static getQueryTextCommonUKRAINEВыплатаЗарплаты() {
        return `SELECT
    Obj.[code] as N_D
    ,CONVERT(MONEY, JSON_VALUE(Obj.doc, '$.Amount')) as SUMMA
    ,BAComp.[code] as COUNT_A
    ...
    static getQueryTextCommonRUSSIAОплатаПоставщику() {
        return `     SELECT
    ...
    static getQueryTextCommonВыплатаЗарплатыСАМОЗАНЯТОМУСОТРУДНИКУ() {
        return `     SELECT
    N'Платежное поручение' as N'СекцияДокумент'
    ,Obj.[code] as N'Номер'
    ,FORMAT (Obj.[date], 'dd.MM.yyyy') as N'Дата'
    ...
    static getQueryTextCommonRUSSIAВыдачаДСПодотчетнику() {
        return `     SELECT
    N'Платежное поручение' as N'СекцияДокумент'
    ,Obj.[code] as N'Номер'
    ,FORMAT (Obj.[date], 'dd.MM.yyyy') as N'Дата'
    ...
    static getQueryTextCommonRUSSIAНаРасчетныйСчетВПуть() {
        return `      SELECT
        N'Платежное поручение' as N'СекцияДокумент'
        ,Obj.[code] as N'Номер'
        ,FORMAT (Obj.[date], 'dd.MM.yyyy') as N'Дата'
    ...
    static getQueryTextCommonRUSSIAВозвратКредитовИЗаймов() {
        return `     SELECT
    N'Платежное поручение' as N'СекцияДокумент'
    ,Obj.[code] as N'Номер'
    ,FORMAT (Obj.[date], 'dd.MM.yyyy') as N'Дата'
    ...
    static getQueryTextCommonRUSSIAперечислениеНалоговИВзносов() {
        return `SELECT
    N'Платежное поручение' as N'СекцияДокумент'
    ,Obj.[code] as N'Номер'
    ,FORMAT (Obj.[date], 'dd.MM.yyyy') as N'Дата'
    ...
    static getQueryTextCommonRUSSIAВыплатаЗарплаты() {
        return `SELECT
    N'Платежное поручение' as N'СекцияДокумент'
    ,Obj.[code] as N'Номер'
    ,FORMAT (Obj.[date], 'dd.MM.yyyy') as N'Дата'
    ...
    static getQueryTextCommonKAZAKHSTANОплатаПоставщику() {
        return `SELECT
    ...
    static async executeQuery(query, params = []) {
        ...
        return await this.tx.manyOrNone(query, params);
    }
    static async getSalaryProjectEmployeesDataFromJSON(EmployeesDataJSON) {
        return await this.tx.manyOrNone(this.getQueryTextSalaryEmployees(), [EmployeesDataJSON]);
    }
    static async BankStatementDataAsSalaryProjectBankStatementString() {
...
<СчетаПК xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://v8.1c.ru/edi/edi_stnd/109" xsi:type="СчетПК"`;
        ...
        for (const row of employees) {
            ...
            for (const prop of Object.keys(row)) {
                if (prop.search('ig_') !== -1 || !row[prop] || (prop.search('rp_') !== -1 && !common[row[prop]]))
                    continue;
        ...
        if (common['ig_DocDate'] >= new Date(2020, 5, 1) && !!(common['ig_ВидДохода'].substr(1, 1).trim()))
            result += `\n\t<КодВидаДохода>${common['ig_ВидДохода'].substr(1, 1)}</КодВидаДохода>`;
        result += `\n\t<КонтрольныеСуммы>`;
    ...
    static async BankStatementDataAsCommonBankStatementStringUKRAINE(bankStatementData) {
        let result = '';
    ...
    static async BankStatementDataAsSalaryProjectBankStatementStringUKRAINE(bankStatementData) {
        let result = 'N_D;SUMMA;COUNT_A;NAME_B;COUNT_B;N_P;OKPO_B;SER_PAS_B;NOM_PAS_B';
        const common = bankStatementData[0];
        const EmployeesInJSON = `{"PayRolls":${JSON.stringify(common['ig_PayRolls'])}}`;
        const employees = await this.tx.manyOrNone(this.getQueryTextSalaryEmployeesUKRAINE(), [EmployeesInJSON]);
        const NP = common['N_P'].replace(/\r?\n/g, ' ');
    ...
    static async BankStatementDataAsCommonBankStatementString(bankStatementData) {
        if (!bankStatementData.length)
            return '';
        if (this.isUKRAINE()) {
                ...
                if (prop.search('_ig') === -1) {
                    ...
                    switch (prop) {
                        ...
                        case 'НазначениеПлатежа':
                            ...
                            if (comNazn.length > naznMaxLength)
                                throw new Error(`Превышена максимально допустимая длина назначения платежа в документе №${row['Номер']} на ${comNazn.length - naznMaxLength} символов`);
    ...
    static async init(docsID, tx) {
        if (!docsID.length)
            return 0;
        const doc = await std_lib_1.lib.doc.byId(docsID[0], tx);
        if (!doc)
            return 0;
        const parentCompany = await std_lib_1.lib.doc.Ancestors(doc.company, tx, 1);
        const country = await std_lib_1.lib.util.getObjectPropertyById(parentCompany, 'Country.id', tx);
        if (!country)
            throw new Error(`Не удалось определить страну организации ${(await std_lib_1.lib.doc.byId(parentCompany, tx)).description}`);
    ...
    WHERE d.id in (@p1)`;
        this.docsByTypes = await this.executeQuery(query);
        this.operationTypes = [...new Set(this.docsByTypes.map(x => x.operation))];
        return this.operationTypes.length;
    }
    static async getBankStatementAsStringWithRules(docsID, tx) {
        if (!docsID.length)
            return '';
        const doc = await std_lib_1.lib.doc.byId(docsID[0], tx);
        if (!doc)
            return '';
        const rules = await std_lib_1.lib.doc.createDocServerById(this.exportRulesOperationId, tx);
        if (!rules)
            return '';
        ...
        if (this.exportRules)
            return await this.exportRules['serverModule']['exportOperationsToString'](this.docsIdsString);
        return '';
    }
    static async getBankStatementAsString(docsID, tx) {
        if (!docsID.length)
            return '';
        ...
        if (result.length && !this.isUKRAINE())
            result += 'КонецФайла';
        return result;
    }
    static async getBankStatementAsStringForOperationType(operationType) {
    ...
    WHERE oper.[id] in (@p1) and JSON_VALUE(oper.doc, '$.Operation') = '433D63DE-D849-11E7-83D2-2724888A9E4F' -- С р/с - на расчетный счет  (в путь)`;
        ...
        return ba.filter(e => e).map(e => `РасчСчет=${e.BankAccount}`).join('\n');
    }
    static async getHeaderFields() {
        const query = `
    SELECT
    ...
    static getShortDocNumber(docNumber, withZeros) {
...
exports.BankStatementUnloader = BankStatementUnloader;
BankStatementUnloader.docsIdsString = '';
BankStatementUnloader.isSalaryProject = false;
BankStatementUnloader.country = '';
BankStatementUnloader.operation = '';
BankStatementUnloader.exportRulesOperationId = 'D3EB3550-0C7B-11EB-A80F-8FEBE5355E8B';
//# sourceMappingURL=BankStatementUnloader.js.map