...
import { RegisterAccumulationOptions, RegisterInfoOptions, RegisterAccumulation, RegisterInfo, AccountRegister } from 'jetti-middle';
...
router.get('/register/info/list/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await sdb.tx(async tx => {
      const result = await tx.manyOrNone<{ type: string }>(query);
      const list = result.map(r => {
